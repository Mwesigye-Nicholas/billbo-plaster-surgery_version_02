...
import calculateAge, { type AgeBreakdown } from "../../../utils/calculateAge.js";
...
const updatePatientDataController = async (
  req: Request<{ patientId: string }, any, PatientFieldsInput>,
  res: Response,
  next: NextFunction,
) => {
  ...
  try {
    ...
    const validateDateOfSurgery =
      req.body.dateOfSurgery !== undefined
        ? isValidFutureOrTodayDate(req.body.dateOfSurgery)
        : undefined;
    const input: PatientFieldsInput = {};
    // Bio fields
    ...
    let updatedAge : AgeBreakdown | undefined;
    ...
    const updatedMedicalDataPayLoad: Record<string, unknown> = {};
    ...
    const updatedPatientBioDataPayLoad: Record<string, unknown> = {};
    ...
    //* update medical data.
    const updatedMedical = await patientMedicalData.findByIdAndUpdate(
      medicalDataId,
      updatedMedicalDataPayLoad,
      { new: true, session },
    );
    ...
    if (!updatedMedical) {
      await session.abortTransaction();
      return next(
        new AppError("Failed to update, patient's medical data", 500),
      );
    }
    //* update bio data.

    const updatedBio = await patientBioData.findByIdAndUpdate(
      patientBioDataId,
      updatedPatientBioDataPayLoad,
