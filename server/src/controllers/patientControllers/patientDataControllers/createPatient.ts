...
const createPatientController = async (
  ...
  const cleaned: any = cleanPatientFields({
    name,
    dateOfBirth,
    sex,
    address,
  ...
  const {
    ...
    cleanedDiagnosis,
    cleanedStatus,
    cleanedPlannedProcedure,
    cleanedDateOfSurgery,
    cleanedDateOfVisit,
    cleanedMedicines,
    cleanedMedicalAndSurgicalHistory,
  } = cleaned;
  ...
  try {
    ...
    const patientExists = await patientBioData.findOne(
      {
        $or: [
          {
            "phoneNumbers.number": {
              $in: cleanedPhoneNumbers.map((p: PhoneNumbers) => p.number),
            },
          },
          { email: cleanedEmail },
        ],
    ...
    if (!bioDataRecord[0]) {
      ...
      session.endSession();
      return next(
        new AppError("Failed to create patient bio data  record", 500),
      );
    }
