...
const cleanPatientFieldsForUpdate = (
  fields: PatientFieldsInput,
): PatientReqObj => {
  const cleaned: PatientReqObj = {};
  ...
  if (fields.medicalAndSurgicalHistory !== undefined) {
    if (fields.medicalAndSurgicalHistory.conditions.length === 0) {
      throw new AppError(
        "Medical and Surgical conditions cannot be empty",
        400,
      );
    }
    ...
    cleaned.cleanedMedicalAndSurgicalHistory = {
      conditions: fields.medicalAndSurgicalHistory.conditions.map((c) =>
        c.trim(),
      ),
