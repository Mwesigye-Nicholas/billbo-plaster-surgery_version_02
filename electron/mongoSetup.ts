...
export async function initializeReplicaSet() {
  ...
  let admin: ReturnType<MongoClient["db"]> | null = null;
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      const primary = members.find((m: any) => m.state === 1 && m.health === 1);
      ...
      if (isFullyReady) {
        console.log(
          "Replica set FULLY READY (PRIMARY stable + healthy members)",
        );
        return;
      }
